...
import {IOka} from "contracts/interfaces/IOka.sol";
...
/// @title MerkleClaim
/// @notice Claims OKA for members of a merkle tree
...
contract MerkleClaim is Ownable {
    ...
    constructor(address _ve, bytes32 _merkleRoot, uint256 _duration) {
        ...
        OKA = IOka(IVotingEscrow(_ve).token());
    ...
    function claim(
        ...
        OKA.approve(address(ve), claimableAmount);
    ...
    function withdrawOKA(address _recipient) external onlyOwner {
        require(block.timestamp > startTime + duration * 86400, "Airdrop is not finished yet");
        uint256 remaining = OKA.balanceOf(address(this));
        require(remaining > 0, "No remaining tokens");
        OKA.transfer(_recipient, remaining);
